...
class StarknetProver extends vm_js_1.AbstractProver {
        ...
        if (fast) {
            return this.cache.get(target, async (a) => {
                try {
                    ...
                    if ((0, utils_js_1.isRPCError)(err, 20))
                        return false;
                    throw err;
                }
            });
    ...
    async getStorage(target, slot, fast = this.fast) {
        ...
        if (storageProof) {
            return (0, utils_js_1.toPaddedHex)(proofEdge(storageProof).path.value);
        }
        if (fast) {
            return this.provider.send('starknet_getStorageAt', [
                target,
                (0, utils_js_1.toPaddedHex)(slot),
                this.blockId,
            ]);
        }
        const proofs = await this.getProofs(target, [slot]);
        return isContract(proofs)
            ? (0, utils_js_1.toPaddedHex)(proofEdge(proofs.contract_proof).path.value)
...
exports.StarknetProver = StarknetProver;
