...
exports.BlockProver = exports.AbstractProver = exports.GatewayVM = exports.GatewayTrace = exports.GatewayRequest = exports.GatewayProgram = exports.CallbackError = void 0;
...
const ERROR_ABI = new abi_1.Interface([`error TooManyProofs(uint256 max)`]);
...
class GatewayProgram {
    ...
    slice(x, n) {
        if (x !== undefined)
            this.push(x).push(n);
        return this.addByte(ops_js_1.GATEWAY_OP.SLICE);
    }
    length() {
    ...
    chunk(n) {
        if (n !== undefined)
            this.push(n);
        return this.addByte(ops_js_1.GATEWAY_OP.CHUNK);
    }
    plus() {
...
class GatewayTrace {
    maxProofCount;
    maxAllocBytes;
    requireTargetBeforeSlot;
    static from(prover) {
        return new this(prover.maxUniqueProofs, prover.maxAllocBytes, prover.requireTargetBeforeSlot);
    }
    needs = [];
    targets = new Map();
    proofBudget;
    allocBudget;
    constructor(maxProofCount = Infinity, maxAllocBytes = Infinity, requireTargetBeforeSlot = true) {
        this.maxProofCount = maxProofCount;
        this.maxAllocBytes = maxAllocBytes;
        this.requireTargetBeforeSlot = requireTargetBeforeSlot;
    ...
    consumeProofs(size) {
        ...
        if (this.proofBudget < 0)
            throw new CallbackError(`too many proofs: ${this.maxProofCount}`, ERROR_ABI.encodeErrorResult('TooManyProofs', [this.maxProofCount]));
    }
...
exports.GatewayTrace = GatewayTrace;
...
class GatewayVM {
    ...
    exitCode = 0;
    outputs;
    constructor(outputs, maxStack = Infinity, trace = new GatewayTrace()) {
...
class AbstractProver {
    ...
    async evalReader(reader) {
        const vm = new GatewayVM(reader.readByte(), // number of outputs
        this.maxStackSize, GatewayTrace.from(this));
        ...
        while (reader.remaining) {
            ...
            switch (op) {
                ...
                case ops_js_1.GATEWAY_OP.READ_BYTES: {
                    const { target, slot } = vm;
                    const { value, slots } = await this.getStorageBytes(target, slot, vm.trace.remainingProvableBytes);
                ...
                case ops_js_1.GATEWAY_OP.READ_ARRAY: {
                    ...
                    const { target, slot } = vm;
                    let length = checkSize(uint256FromHex(await this.getStorage(target, slot)), vm.trace.remainingProvableBytes);
                    if (step < 32) {
                        const per = (32 / step) | 0;
                ...
                case ops_js_1.GATEWAY_OP.CHUNK: {
                    const [v, n] = await Promise.all(vm.popSlice(2).map(wrap_js_1.unwrap));
                    ...
                    if (chunk) {
                        ...
                        while (end >= chunk) {
                            vm.push((0, utils_1.dataSlice)(v, end - chunk, end));
                            end -= chunk;
                        }
                    }
                    continue;
        ...
        return new wrap_js_1.Wrapped(NaN, async () => {
            ...
            if (can !== false) {
                try {
                    ...
                    if (!can && (0, utils_1.isCallException)(err))
    ...
    async getStorageBytes(target, slot, limit) {
