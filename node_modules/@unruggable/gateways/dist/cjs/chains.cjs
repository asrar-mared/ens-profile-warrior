...
exports.chainFromName = chainFromName;
exports.isStarknet = isStarknet;
exports.isL1 = isL1;
// https://chainlist.wtf
...
function chainName(chain) {
    for (const [name, c] of Object.entries(exports.CHAINS)) {
        if (c === chain)
            return name;
    }
    throw new TypeError(`unknown chain: ${chain}`);
}
function chainFromName(slug) {
    if (/^(0x)?[0-9a-f]+$/i.test(slug))
        return BigInt(slug);
    const key = slug.toUpperCase().replaceAll('-', '_');
    if (key in exports.CHAINS)
        return exports.CHAINS[key];
    throw new Error(`unknown chain: ${slug}`);
}
// idea: chainType? chainKind?
// at the moment, the only distinction needed is address type
function isStarknet(chain) {
    switch (chain) {
        case exports.CHAINS.STARKNET:
        case exports.CHAINS.STARKNET_SEPOLIA:
            return true;
    }
    return false;
}
...
function isL1(chain) {
    switch (chain) {
        case exports.CHAINS.MAINNET:
        case exports.CHAINS.SEPOLIA:
        case exports.CHAINS.HOLESKY:
        case exports.CHAINS.HOODI:
        case exports.CHAINS.BSC:
            return true;
    }
    return false;
}
