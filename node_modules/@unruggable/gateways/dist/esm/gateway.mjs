...
const pollMs0 = 60000;
export class Gateway extends EZCCIP {
    rollup;
    ...
    latestCache = new CachedValue(() => this.rollup.fetchLatestCommitIndex(), pollMs0);
    commitCacheMap = new CachedMap(Infinity);
    callLRU = new LRU(callCapacity0);
    constructor(rollup) {
        super();
        this.rollup = rollup;
        this.register(GATEWAY_ABI, {
            proveRequest: async ([ctx, [req]], _context, history) => {
                ...
                const hash = solidityPackedKeccak256(['uint256', 'bytes'], [commit.index, req]);
    ...
    async cachedCommit(index, cacheMs) {
        ...
        return this.commitCacheMap.get(index, async (i) => {
            ...
            const parent = new CachedValue(() => this.rollup.fetchParentCommitIndex(commit), this.latestCache.cacheMs);
            const valid = new CachedValue(() => this.rollup.isCommitStillValid(commit), this.latestCache.cacheMs);
...
export class GatewayV1 extends EZCCIP {
    rollup;
    latestCommit;
    latestCache = new CachedValue(async () => {
