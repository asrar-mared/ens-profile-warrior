import { ZKSyncProver } from './ZKSyncProver.mjs';
import { DIAMOND_ABI } from './types.mjs';
...
export class ZKSyncRollup extends AbstractRollup {
    ...
    constructor(providers, config) {
        super(providers);
        this.DiamondProxy = new Contract(config.DiamondProxy, DIAMOND_ABI, this.provider1);
    }
    ...
    encodeWitness(commit, proofSeq) {
        return ABI_CODER.encode(['(bytes, bytes[], bytes)'], [[commit.abiEncodedBatch, proofSeq.proofs, proofSeq.order]]);
    }
    windowFromSec(sec) {
...
function decodeBatches(txData) {
    const { commitData } = DIAMOND_ABI.decodeFunctionData('commitBatchesSharedBridge', txData);
