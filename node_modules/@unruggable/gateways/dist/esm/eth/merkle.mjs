...
export function verifyAccountState(target, accountProof, stateRoot) {
...
export function verifyStorageValue(slot, storageProof, storageRoot) {
    if (storageRoot === NULL_TRIE_HASH) {
        //if (storageProof.length) throw new Error('expected empty proof');
        return ZeroHash;
    }
    const rlp = verifyMerkleTrieValue(toPaddedHex(slot), storageProof, storageRoot, true);
    if (!rlp)
        return ZeroHash;
...
export function verifyMerkleTrieValue(key, proof, root, secure) {
    try {
        ...
        const { pathLength, keyRemainder, final } = walk(nodes, secure ? keccak256(key) : key, root);
