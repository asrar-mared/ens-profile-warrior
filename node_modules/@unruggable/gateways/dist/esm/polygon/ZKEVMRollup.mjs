...
import { ZKEVMProver } from './ZKEVMProver.mjs';
import { ZeroHash } from 'ethers/constants';
import { ABI_CODER, toUnpaddedHex } from '../utils.mjs';
...
export class ZKEVMRollup extends AbstractRollup {
    RollupManager;
    rollupID;
    ...
    static async create(providers, config) {
        const RollupManager = new Contract(config.RollupManager, ROLLUP_ABI, providers.provider1);
    ...
    encodeWitness(commit, proofSeq) {
        return ABI_CODER.encode(['tuple(uint256, bytes[], bytes)'], [[commit.index, proofSeq.proofs, proofSeq.order]]);
    }
    windowFromSec(sec) {
