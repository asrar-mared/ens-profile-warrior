...
export class LineaRollup extends AbstractRollup {
    ...
    async fetchLatestCommitIndex() {
        ...
        const index = await this.L1MessageService.currentL2BlockNumber({
            blockTag: this.latestBlockTag,
        });
        const timeout = Date.now() + this.shomeiTimeout;
        let commit = await this.fetchCommit(index);
        for (;;) {
            if (await commit.prover.isShomeiReady())
                return commit.index;
            if (Date.now() > timeout)
                throw new Error(`timeout isShomeiReady()`);
            commit = await this.fetchParentCommit(commit);
        }
    }
    async _fetchParentCommitIndex(commit) {
        if (commit.startIndex)
            return commit.startIndex - 1n;
        const [event] = await this.L1MessageService.queryFilter(this.L1MessageService.filters.DataFinalized(null, null, commit.prevStateRoot));
    ...
    async _fetchCommit(index) {
        ...
        if (this.firstCommitV3 && index >= this.firstCommitV3) {
            const [event] = await this.L1MessageService.queryFilter(this.L1MessageService.filters.DataFinalizedV3(null, index));
            if (!(event instanceof EventLog)) {
            ...
            const [event] = await this.L1MessageService.queryFilter(this.L1MessageService.filters.DataFinalized(index));
            if (!(event instanceof EventLog)) {
