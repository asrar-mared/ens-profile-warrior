...
export const encodeNulsAddress = (source) => {
    ...
    if (chainId === 1)
        ...
        const chainIdBytes = concatBytes(new Uint8Array([0xff & (chainId >> 0)]), new Uint8Array([0xff & (chainId >> 8)]));
        prefix = base58UncheckedEncode(chainIdBytes).toUpperCase();
    }
    return (prefix + prefixReference[prefix.length - 1] + base58UncheckedEncode(payload));
};
export const decodeNulsAddress = (source) => {
