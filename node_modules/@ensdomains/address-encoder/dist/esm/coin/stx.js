...
const base32Stx = utils.chain(radixStx, utils.alphabet("0123456789ABCDEFGHJKMNPQRSTVWXYZ"), utils.join(""));
const stxChecksum = (data) => sha256(sha256(data)).slice(0, checkumLength);
export const encodeStxAddress = (source) => {
    ...
    else if (equalBytes(checksum, stxChecksum(concatBytes(p2shVersion, hash160)))) {
        version = "M";
        encoded = base32Stx.encode(source);
    }
    else
...
export const decodeStxAddress = (source) => {
    if (source.length < 6)
        throw new Error("Unrecognised address format");
    if (source[0] !== "S")
        throw new Error("Unrecognised address format");
