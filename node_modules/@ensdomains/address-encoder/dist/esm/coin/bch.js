...
import { decodeBchAddressToTypeAndHash, encodeBchAddressWithVersion, } from "../utils/bch.js";
const name = "bch";
const coinType = 145;
const p2pkhVersions = [new Uint8Array([0x00])];
const p2shVersions = [new Uint8Array([0x05])];
const bchBase58Decode = createBase58VersionedDecoder(p2pkhVersions, p2shVersions);
export const encodeBchAddress = (source) => {
    if (source[0] === 0x76) {
        if (source[1] !== 0xa9 ||
...
export const decodeBchAddress = (source) => {
    try {
        ...
        if (type === 0)
            return concatBytes(new Uint8Array([0x76, 0xa9, 0x14]), new Uint8Array(hash), new Uint8Array([0x88, 0xac]));
        if (type === 8)
            return concatBytes(new Uint8Array([0xa9, 0x14]), new Uint8Array(hash), new Uint8Array([0x87]));
        throw new Error("Unrecognised address format");
    }
};
export const bch = {
