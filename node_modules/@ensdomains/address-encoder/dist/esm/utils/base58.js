...
export const createBase58VersionedEncoder = (p2pkhVersion, p2shVersion) => (source) => {
    ...
    if (source[0] === 0x76) {
        ...
        return base58CheckEncode(concatBytes(p2pkhVersion, source.slice(3, 3 + source[2])));
    }
    ...
    if (source[0] === 0xa9) {
        ...
        return base58CheckEncode(concatBytes(p2shVersion, source.slice(2, 2 + source[1])));
    }
    throw new Error("Unrecognised address format");
};
export const createBase58VersionedDecoder = (p2pkhVersions, p2shVersions) => (source) => {
    ...
    if (p2pkhVersions.some(checkVersion))
        return concatBytes(new Uint8Array([0x76, 0xa9, 0x14]), addr.slice(p2pkhVersions[0].length), new Uint8Array([0x88, 0xac]));
    if (p2shVersions.some(checkVersion))
        return concatBytes(new Uint8Array([0xa9, 0x14]), addr.slice(p2shVersions[0].length), new Uint8Array([0x87]));
    throw new Error("Unrecognised address format");
};
