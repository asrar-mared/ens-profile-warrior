...
const toBeRevertedWithStringMessage = ({ expectedReasonString, isRegExp, negated, why, }) => {
    const expectedString = `${negated ? "NOT " : ""}transaction reverted with string: ${isRegExp ? `matching ${expectedReasonString}` : expectedReasonString}`;
    return [
        "",
        matcherHint({
            matcherName: `.${TO_BE_REVERTED_WITH_STRING_MATCHER}`,
            expected: `${isRegExp ? expectedReasonString : `"${expectedReasonString}"`}`,
            isNot: negated,
        }),
        "",
...
export function supportRevertedWithString(chai) {
    addMethod(chai, TO_BE_REVERTED_WITH_STRING_MATCHER, async function (expectedReason) {
        ...
        const onError = (error) => {
            ...
            assert({
                condition: returnData.kind === "error" &&
