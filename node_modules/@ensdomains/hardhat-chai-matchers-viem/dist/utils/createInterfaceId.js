 ...
 */
export const getSolidityReferenceInterfaceAbi = async (interfaceOrFullyQualifiedName) => {
    const artifact = await hre.artifacts.readArtifact(interfaceOrFullyQualifiedName);
    const fullyQualifiedNames = await hre.artifacts
        .getAllFullyQualifiedNames()
        .then((names) => Array.from(names));
    const fullyQualifiedInterfaceName = fullyQualifiedNames.find((n) => n.endsWith(interfaceOrFullyQualifiedName));
    if (!fullyQualifiedInterfaceName)
        throw new Error("Couldn't find fully qualified interface name");
    const { buildInfoId, inputSourceName } = await hre.artifacts.readArtifact(fullyQualifiedInterfaceName);
    if (!buildInfoId)
...
//# sourceMappingURL=createInterfaceId.js.map